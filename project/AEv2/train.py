...
if __name__ == "__main__":
    ...
    # AEv2 model
    if not os.path.exists('aev2.h5'):
        aev2 = AEv2()
        model_opt = Adam(lr=0.001)

        aev2.compile(optimizer=model_opt, loss='mae', metrics=['accuracy'])
        print("Creat new AEv2 model!")
        aev2.summary()

    else:
        aev2 = load_model("aev2.h5")  # load model de train tiep
    ...
    checkpoint_best = ModelCheckpoint("aev2_best.h5", monitor='val_loss', save_best_only=True, mode='min', verbose=1)
